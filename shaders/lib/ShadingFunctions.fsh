...
float ComputeDirectSunlight(in vec4 position, in float normalShading) {
	...
	||  position.z < 0.0 || position.z > 1.0
		...
		mat2 blockerRotation = mat2(cos(angle.x), -sin(angle.x),
		                            sin(angle.y),  cos(angle.y)); //Random Rotation Matrix for blocker, high noise.

		mat2 blurRoatation = mat2(cos(angle.x), -sin(angle.x),
		                          sin(angle.x),  cos(angle.x)); //Random Rotation Matrix for bluring, low noise
		...
		for (float i = -range; i <= range; i++) {
			for (float j = -range; j <= range; j++) {
				vec2 coord = vec2(i, j) * blurRoatation;
